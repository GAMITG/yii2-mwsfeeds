...
namespace atomarweb\mwsfeeds;

//use atomarweb\mwsfeeds\RequestType;
//use atomarweb\mwsfeeds\MarketplaceWebService_Interface;
//use atomarweb\mwsfeeds\MarketplaceWebService_Exception;
use atomarweb\mwsfeeds\Model\MarketplaceWebService_Model_ResponseHeaderMetadata;
use atomarweb\mwsfeeds\Model\MarketplaceWebService_Model_SubmitFeedRequest;
use atomarweb\mwsfeeds\Model\MarketplaceWebService_Model_SubmitFeedResponse;
use atomarweb\mwsfeeds\Model\MarketplaceWebService_Model_GetFeedSubmissionResultRequest;
use atomarweb\mwsfeeds\Model\MarketplaceWebService_Model_GetFeedSubmissionResultResponse;
...
class MarketplaceWebService_Client implements MarketplaceWebService_Interface
{
  ...
  private function performRequest($action, array $converted, $dataHandle = null) {
    ...
    if ($this->getRequestType($action) === self::POST_DOWNLOAD) {
    	if ($code != 200) {
   ...
   * with self::POST_DOWNLOAD request types, since the responses can potentially become
   * large.
   *
  ...
  private function configureCurlOptions($action, array $converted, $streamHandle = null) {
    ...
    $requestType = $this->getRequestType($action);

    if ($requestType == self::POST_UPLOAD) {
    ...
    } else if (!($requestType === self::UNKNOWN)) {
      ...
      if ($requestType == self::POST_DOWNLOAD) {
  ...
  /**
   * For self::POST_DOWNLOAD actions, construct a response containing the Amazon Request ID
